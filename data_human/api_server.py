...
def param(path: str):
    ...
    parser.add_argument('--background_enhancer',  type=str, default="realesrga", help="background enhancer, [realesrgan]")
...
@app.post("/vidio")
def no_params_example(path :Item):
    #torch.backends.cudnn.enabled = False
    args=param(path.path)
    pic_path = args.source_image
    audio_path = args.driven_audio
    save_dir = os.path.join(args.result_dir, strftime("%Y_%m_%d_%H.%M.%S"))
    os.makedirs(save_dir, exist_ok=True)
    ...
    if not args.verbose:
        shutil.rmtree(save_dir)
    result_dict = {"code": 0, "msg": "ok", "res": save_dir+'.mp4'}

    return result_dict
   
if __name__ == '__main__':
    uvicorn.run("api_server:app", host='127.0.0.1', port=7864)